...
getFilesFromWD <- function (wd,csvName) {
  setwd(wd)
  filesList <- list.files()
  if (!is.na(csvName)) {
    for(i in length(filesList)) {
      if(filesList[i]==csvName) 
        id = i
      filesList[id] <- NA
      filesList <- filesList[complete.cases(filesList)]
    }
  }
  return(filesList)
}

getByIDSFromWD <- function(wd,list, hero_id ){
  setwd(wd)
  library(jsonlite)
  temp <- list()
  retorno <- list()
  ...
  for (i in 1:length(list)){
    ...
    tryCatch({
      data <- fromJSON(list[i], flatten=TRUE)
      temp = getVariables(data,hero_id)
      retorno <- rbind(retorno,temp)
      kills <- getKills(data,hero_id)
...
getByIDSFromURI <- function(list ){
  ...
  for (i in 1:length(list)){
    ...
    tryCatch({
      data2 <- fromJSON(paste(link,list[1],sep=""), flatten=TRUE)
...
getByIDS <- function(list, hero_id ){
  setwd("C:\\Users\\gabri\\git\\APIDotaMiner\\apiMinerDota\\files\\out\\byHero\\test\\00-4")
  library(jsonlite)
  link <- "https://api.opendota.com/api/matches/"
  temp <- list()
...
getVariables <- function(data, hero_id) {
  for(i in 1:length(data$players$hero_id)) { if(data$players$hero_id[i]==hero_id) id = i}
  temp <- list()
  ...
  # temp$player.lane                                <- data$players$lane[id]da
...
getKills<- function (data, hero_id) {
  for(i in 1:length(data$players$hero_id)){if(data$players$hero_id[i]==hero_id) id = i}
  temp <- list()
  temp$match_id <- data$match_id
  temp$player.hero_id                               <- data$players$hero_id[id]
